...
/datum/game_mode/proc/announce_ending()
	...
	to_chat(world, narrate_body("The second batch of Test Crews arrived at the PST with an unexpected guide. Together, they make it back to the Crypt where the previous batch found their legacy."))
	to_chat(world, narrate_body("There, Commander Reed used her familial ties to break the safeguards preventing the entity known as Pythia from taking control of the PST. Once this failsafe is gone, the PST fully activates in all its glory, releasing thousands of automated drones to catch up to a thousand scheduled maintenance tasks."))
	to_chat(world, narrate_body("To the outside world, the Outer Veil Primary Supply Terminal now has a revolutionary AI. But for the members of the Test Crews, this is just the beginning."))
	sleep(50)
