/obj/structure/eventterminal/briefing_terminal
	...
	plane = GAME_PLANE
	var/briefing_playing = FALSE
	icon_state = "open_ok"
	puzzlebox_id = "historylessons"
	item_serial = "UACM-OVPST-PEAK-ORIENTATION-TERM"

/obj/structure/eventterminal/briefing_terminal/attack_hand(mob/user)
	if(!puzzlebox_user)
	...
	if(puzzlebox_user != usr.real_name)
		...
		return
	if (briefing_playing == TRUE)
		terminal_speak("Briefing file playback in progress. Please wait until current briefing is complete.")
	if(!("briefing_welcome" in usr.saw_narrations))
		terminal_welcome()
	terminal_cmd()
	puzzlebox_user = null
	return

/obj/structure/eventterminal/briefing_terminal/proc/terminal_welcome()
	terminal_speak("Welcome, Test Crew members.")
	terminal_speak("If this is the first time, you're seeing this room and this terminal, this likely means that you are fresh off whatever ship brought you here, possibly wondering what this place is and what you'll be doing here.")
	terminal_speak("You're in luck. The recordings on this very terminal, left here by your future COs and other people of note you'll likely be encountering during your work here that should get you up to speed and what we do here and what your job will be.")
	terminal_speak("We will try to keep this updated and add to them over time, hopefully this very terminal should serve as your own little refresher course on your duties should you ever need it.")
	terminal_speak("If you are new to the PST and the Test Crews, I highly recommend you at least view the basic four courses that are currently available, as they are the core basics of your future missions.")
	terminal_speak("Use the LIST command to see available entries or a number to start watching a specific entry.")
	terminal_speak("Good luck. And if I haven't had a chance to do this in person - welcome to the Test Crews and the PST.")
	terminal_speak("-C.")
	if(!("buffer_welcome" in usr.saw_narrations))
		usr.saw_narrations.Add("briefing_welcome")

/obj/structure/eventterminal/briefing_terminal/proc/terminal_cmd()
...
/obj/structure/eventterminal/briefing_terminal/proc/terminal_list()
	terminal_speak("Please select lesson by number. No input or a zero will bring you back to the main command input prompt.")
...
/obj/structure/eventterminal/briefing_terminal/proc/terminal_play(str)
	var/puzzlebox_parser_input = str
	switch(puzzlebox_parser_input)
		if("1")
			talkas("Playing presentation 1.")
			for (var/obj/structure/eventterminal/briefing_display/T in world)
				INVOKE_ASYNC(T, TYPE_PROC_REF(/obj/structure/eventterminal/briefing_display, lesson1))
				puzzlebox_user = null
				return
		if("2")
			talkas("Playing presentation 2.")
			for (var/obj/structure/eventterminal/briefing_display/T in world)
				INVOKE_ASYNC(T, TYPE_PROC_REF(/obj/structure/eventterminal/briefing_display, lesson2))
				puzzlebox_user = null
				return
		if("3")
			talkas("Playing presentation 3.")
			for (var/obj/structure/eventterminal/briefing_display/T in world)
				INVOKE_ASYNC(T, TYPE_PROC_REF(/obj/structure/eventterminal/briefing_display, lesson3))
				puzzlebox_user = null
				return
		if("4")
			talkas("Playing presentation 4.")
			for (var/obj/structure/eventterminal/briefing_display/T in world)
				INVOKE_ASYNC(T, TYPE_PROC_REF(/obj/structure/eventterminal/briefing_display, lesson4))
				puzzlebox_user = null
				return
		else
...
/obj/structure/eventterminal/briefing_display/attack_hand(mob/user as mob)
