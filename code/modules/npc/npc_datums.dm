	...
	switch(get_dir(starting_turf,ending_turf))
		if(NORTH)
			...
			for(var/atom/atom_to_test_1 in new_turf)
				if(atom_to_test_1.density == 1|| istype(atom_to_test_1, /obj/structure/shuttle/part) || istype(new_turf,/turf/closed))
					new_turf = locate(ending_turf.x - 1,ending_turf.y + 1,ending_turf.z)
					for(var/atom/atom_to_test_2 in new_turf)
						if(atom_to_test_2.density == 1|| istype(atom_to_test_2, /obj/structure/shuttle/part) || istype(new_turf,/turf/closed))
							new_turf = locate(ending_turf.x + 1,ending_turf.y,ending_turf.z)
							for(var/atom/atom_to_test_3 in new_turf)
								if(atom_to_test_3.density == 1|| istype(atom_to_test_3, /obj/structure/shuttle/part) || istype(new_turf,/turf/closed))
									new_turf = locate(ending_turf.x - 1,ending_turf.y,ending_turf.z)
									for(var/atom/atom_to_test_4 in new_turf)
										if(atom_to_test_4.density == 1|| istype(atom_to_test_4, /obj/structure/shuttle/part) || istype(new_turf,/turf/closed)) return 1
		if(NORTH,SOUTH)
			new_turf = locate(ending_turf.x + 1,ending_turf.y - 1,ending_turf.z)
			for(var/atom/atom_to_test_1 in new_turf)
				if(atom_to_test_1.density == 1|| istype(atom_to_test_1, /obj/structure/shuttle/part) || istype(new_turf,/turf/closed))
